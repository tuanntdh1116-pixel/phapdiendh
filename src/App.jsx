...
import { store as localDB, cloudReady, uploadAttachment } from "./cloudStore.js";
...
// Định dạng ngày từ "yyyy-mm-dd" (giá trị input type=date) sang "dd/mm/yyyy" để hiển thị.
// Giữ nguyên nếu không đúng định dạng (VD: "—" khi chưa có ngày).
...
function seedDocuments() {
  return [
    ...
    {
      id: "d8", code: "BM.KH-2024-014", title: "Biểu mẫu đề xuất kế hoạch bảo dưỡng định kỳ (bản mới)",
      category: "bieu-mau", department: "Phòng Kế hoạch - Vật tư", status: "hieu-luc", version: "1.0",
      author: "Trịnh Thu Hà", approver: "PGĐ Phạm Quốc Bảo",
      issueDate: "2024-03-01", effectiveDate: "2024-03-15",
      summary: "Mẫu biểu đề xuất kế hoạch bảo dưỡng định kỳ, thay thế biểu mẫu BM.KH-2021-007, bổ sung thêm cột dự trù vật tư.",
      replaces: "BM.KH-2021-007",
      history: [
        { date: "2024-02-10", action: "Soạn thảo", by: "Trịnh Thu Hà", note: "" },
        { date: "2024-03-01", action: "Ban hành", by: "Văn thư Công ty", note: "Thay thế BM.KH-2021-007" },
      ],
    },
    {
...
// Dựng chuỗi liên kết "thay thế" quanh một văn bản: đi ngược theo `replaces`
// để tìm các văn bản đã bị nó thay thế (bên trái), và đi tới theo `replacedBy`
// để tìm các văn bản đã thay thế nó (bên phải). Trả về mảng theo thứ tự
// thời gian, dùng để vẽ sơ đồ mind map dạng chuỗi.
function buildReplacementChain(documents, docId) {
  const byCode = new Map((documents || []).map(d => [d.code.trim().toLowerCase(), d]));
  const start = (documents || []).find(d => d.id === docId);
  if (!start) return [];

  const before = [];
  let cur = start;
  const seenBefore = new Set([cur.id]);
  while (cur.replaces) {
    const prev = byCode.get(cur.replaces.trim().toLowerCase());
    if (!prev || seenBefore.has(prev.id)) break;
    before.unshift(prev);
    seenBefore.add(prev.id);
    cur = prev;
  }

  const after = [];
  cur = start;
  const seenAfter = new Set([cur.id]);
  while (cur.replacedBy) {
    const next = byCode.get(cur.replacedBy.trim().toLowerCase());
    if (!next || seenAfter.has(next.id)) break;
    after.push(next);
    seenAfter.add(next.id);
    cur = next;
  }
...
// Xuất báo cáo tổng hợp số lượng văn bản theo từng loại (danh mục) ra file Excel,
// dùng cho mục Lược đồ văn bản.
function exportCategorySummaryExcel(counts, categoryOptions, documents) {
  const total = (documents || []).length;
  const rows = categoryOptions.map(c => {
    const n = counts[c.key] || 0;
    return {
      "Loại văn bản": c.label,
      "Mã tiền tố": c.prefix || "",
      "Số lượng": n,
      "Tỉ lệ (%)": total ? Math.round((n / total) * 1000) / 10 : 0,
    };
  });
  rows.push({
    "Loại văn bản": "TỔNG CỘNG", "Mã tiền tố": "", "Số lượng": total, "Tỉ lệ (%)": 100,
  });

  const ws = XLSX.utils.json_to_sheet(rows);
  ws["!cols"] = [{ wch: 34 }, { wch: 14 }, { wch: 12 }, { wch: 12 }];

  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, ws, "Danh mục văn bản");

  const today = new Date().toISOString().slice(0, 10);
  XLSX.writeFile(wb, `bao-cao-danh-muc-van-ban-${today}.xlsx`);
}

function findByCode(documents, code) {
  if (!code) return null;
  const target = code.trim().toLowerCase();
  return (documents || []).find(d => d.code.trim().toLowerCase() === target) || null;
}
    ...
    <div className="app">
      ...
      <aside className="sidebar">
        ...
        <div className="legend">
          <div className="legend-title">Tình trạng văn bản</div>
          {STATUSES.map(s => (
      ...
      <main className="main">
        ...
        {documents && tab === "thong-ke" && role === "admin" && (
          <StatsView documents={documents} monthlyStats={monthlyStats} topDocs={topDocs} events={events} />
        )}
      </main>
    </div>
  );
...
function StatsView({ documents, monthlyStats, topDocs, events }) {
  ...
  // Văn bản mới đăng tải trong tháng: dựa vào mốc thời gian tạo (bản ghi lược sử đầu tiên).
  // Văn bản hết hiệu lực trong tháng: dựa vào lược sử có hành động "Thu hồi" / "Bị thay thế".
  const newDocsThisMonth = (documents || []).filter(
    d => d.history && d.history[0] && d.history[0].date && d.history[0].date.slice(0, 7) === thisMonth
  ).length;
  const expiredDocsThisMonth = (documents || []).filter(d =>
    (d.history || []).some(
      h => (h.action === "Thu hồi" || h.action === "Bị thay thế") && h.date && h.date.slice(0, 7) === thisMonth
    )
  ).length;

  const monthlyDocStats = useMemo(() => {
    const byMonth = {};
    const touch = (m) => {
      if (!byMonth[m]) byMonth[m] = { month: m, "Văn bản mới đăng tải": 0, "Văn bản hết hiệu lực": 0 };
      return byMonth[m];
    };
    (documents || []).forEach(d => {
      if (d.history && d.history[0] && d.history[0].date) {
        touch(d.history[0].date.slice(0, 7))["Văn bản mới đăng tải"]++;
      }
      (d.history || []).forEach(h => {
        if ((h.action === "Thu hồi" || h.action === "Bị thay thế") && h.date) {
          touch(h.date.slice(0, 7))["Văn bản hết hiệu lực"]++;
        }
      });
    });
    return Object.values(byMonth).sort((a, b) => a.month.localeCompare(b.month)).slice(-12);
  ...
  return (
    <div className="view">
      ...
      <div className="stat-cards">
        ...
        <div className="stat-card">
          <span className="stat-num mono">{newDocsThisMonth}</span>
          <span className="stat-label">Văn bản mới đăng tải (tháng này)</span>
        </div>
        <div className="stat-card">
          <span className="stat-num mono">{expiredDocsThisMonth}</span>
          <span className="stat-label">Văn bản hết hiệu lực (tháng này)</span>
        </div>
      </div>
      ...
      <div className="chart-panel" style={{ marginTop: "1.5rem" }}>
        <h2 className="sub-title">Văn bản mới đăng tải / hết hiệu lực theo tháng</h2>
        {monthlyDocStats.length === 0 ? (
          <div className="empty">Chưa có dữ liệu văn bản để thống kê.</div>
        ) : (
          <div style={{ width: "100%", height: 320 }}>
            <ResponsiveContainer>
              <BarChart data={monthlyDocStats}>
                <CartesianGrid strokeDasharray="3 3" stroke="#D8DEE4" />
                <XAxis dataKey="month" tick={{ fontSize: 11, fontFamily: "IBM Plex Mono, monospace" }} />
                <YAxis allowDecimals={false} tick={{ fontSize: 11 }} />
                <Tooltip />
                <Legend />
                <Bar dataKey="Văn bản mới đăng tải" fill="#1F8A70" radius={[3, 3, 0, 0]} />
                <Bar dataKey="Văn bản hết hiệu lực" fill="#C0392B" radius={[3, 3, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        )}
...
function SearchView({ documents, filtered, q, setQ, fCategory, setFCategory, fDept, setFDept, fStatus, setFStatus, onSelect, selectedDoc, onCloseDetail, onGoHistory, onDelete, onEdit, onRevoke, categoryOptions, departmentOptions, total }) {
    ...
    <div className="view">
      ...
      <div className="filter-bar">
        ...
        <input
          ...
          placeholder="Tất cả tình trạng văn bản (gõ để lọc)"
        />
        ...
        <div className="modal-backdrop" onClick={onCloseDetail}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-grid">
              ...
              <div><span className="k">Ngày ban hành</span><span className="v mono">{formatDateVN(selectedDoc.issueDate)}</span></div>
              <div><span className="k">Ngày hiệu lực</span><span className="v mono">{formatDateVN(selectedDoc.effectiveDate)}</span></div>
              {selectedDoc.replaces && (() => {
                const target = findByCode(documents, selectedDoc.replaces);
                return (
                  <div>
                    <span className="k">Thay thế cho</span>
                    {target ? (
                      <button type="button" className="v v-link" onClick={() => onSelect(target.id)}>
                        <span className="mono">{target.code}</span> — {target.title}
                      </button>
                    ) : (
                      <span className="v mono">{selectedDoc.replaces}</span>
                    )}
                  </div>
                );
              })()}
              {selectedDoc.replacedBy && (() => {
                const target = findByCode(documents, selectedDoc.replacedBy);
                return (
                  <div>
                    <span className="k">Đã được thay thế bởi</span>
                    {target ? (
                      <button type="button" className="v v-link" onClick={() => onSelect(target.id)}>
                        <span className="mono">{target.code}</span> — {target.title}
                      </button>
                    ) : (
                      <span className="v mono">{selectedDoc.replacedBy}</span>
                    )}
                  </div>
                );
              })()}
            ...
            {selectedDoc.related && selectedDoc.related.length > 0 && (
              <div className="related-tags">
                <span className="related-tags-title">Văn bản liên quan</span>
                <div className="related-tags-list">
                  {selectedDoc.related.map(code => {
                    const target = findByCode(documents, code);
                    ...
                    return (
                      <button
                        ...
                        style={{ "--node-color": categoryOf(target.category).color }}
...
function emptyFormState() {
  return {
    ...
    summary: "", attachmentFile: null, removeAttachment: false, replaces: "", related: [],
  };
}
...
function formFromDoc(doc) {
  return {
    ...
    replaces: doc.replaces || "", related: doc.related || [],
  };
}
  ...
  return (
    <div className="view">
      ...
      <form className="upload-form" onSubmit={handleSubmit}>
        ...
        <label className="full">Văn bản liên quan
          <div className="related-picker">
            {(documents || []).filter(d => !isEdit || d.id !== initialDoc.id).length === 0 && (
              <span className="field-hint">Chưa có văn bản nào khác trong hệ thống để chọn.</span>
            )}
            {(documents || [])
              .filter(d => !isEdit || d.id !== initialDoc.id)
              .map(d => (
                <label key={d.id} className="related-picker-item">
                  <input
                    type="checkbox"
                    checked={form.related.includes(d.code)}
                    onChange={e => {
                      const checked = e.target.checked;
                      update("related", checked
                        ? [...form.related, d.code]
                        : form.related.filter(c => c !== d.code));
                    }}
                  />
                  <span><span className="mono">{d.code}</span> — {d.title}</span>
                </label>
              ))}
          </div>
          <span className="field-hint">Tích chọn các văn bản có nội dung liên quan (không nhất thiết là quan hệ thay thế) — sẽ hiện trong Sơ đồ liên kết văn bản.</span>
        </label>
...
function LinkMindMap({ documents }) {
  const linkedDocs = useMemo(
    () => (documents || []).filter(d => d.replaces || d.replacedBy || (d.related && d.related.length)),
    [documents]
  );
  const [selectedId, setSelectedId] = useState(linkedDocs[0]?.id ?? null);

  useEffect(() => {
    if (!linkedDocs.some(d => d.id === selectedId)) {
      setSelectedId(linkedDocs[0]?.id ?? null);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [linkedDocs]);

  const chain = useMemo(() => buildReplacementChain(documents, selectedId), [documents, selectedId]);
  const selectedDoc = (documents || []).find(d => d.id === selectedId) || null;
  const relatedDocs = useMemo(
    () => (selectedDoc?.related || []).map(code => findByCode(documents, code)).filter(Boolean),
  ...
  if (linkedDocs.length === 0) {
    return (
      <div className="empty" style={{ marginTop: "1rem" }}>
        Chưa có văn bản nào khai báo liên kết. Khi đăng tải/sửa văn bản, chọn mục
        "Thay thế cho văn bản/quyết định số" hoặc tích chọn "Văn bản liên quan" để hệ thống tự vẽ sơ đồ liên kết tại đây.
      </div>
    );
  }
  ...
  return (
    <div className="mindmap-wrap">
      <label className="mindmap-picker">
        Chọn văn bản để xem sơ đồ liên kết
      ...
      <div className="mindmap">
        {chain.map((d, i) => (
          <div className="mindmap-node-row" key={d.id}>
            <div
              className={"mindmap-node" + (d.id === selectedId ? " active" : "")}
              style={{ "--node-color": categoryOf(d.category).color }}
            >
              <CodeChip code={d.code} color={categoryOf(d.category).color} />
              <div className="mindmap-node-title">{d.title}</div>
              <div className="mindmap-node-meta mono">v{d.version} · {statusOf(d.status).label}</div>
            </div>
            {i < chain.length - 1 && <div className="mindmap-arrow">→</div>}
          </div>
        ))}
      </div>
      <p className="field-hint" style={{ marginTop: "0.75rem" }}>
        Mũi tên thể hiện chiều thay thế: văn bản bên trái bị thay thế bởi văn bản bên phải.
      </p>

      {relatedDocs.length > 0 && (
        <div className="mindmap-related">
          <div className="legend-title" style={{ marginTop: "1.75rem" }}>
            Văn bản liên quan tới {selectedDoc.code}
          </div>
          <div className="mindmap-related-list">
            {relatedDocs.map(d => (
              <button
                key={d.id}
                type="button"
                className="mindmap-related-chip"
                style={{ "--node-color": categoryOf(d.category).color }}
...
function SchemaView({ documents, counts, categoryOptions, onPickCategory }) {
  return (
    <div className="view">
      <div className="view-head">
        ...
        <button
          ...
          onClick={() => exportCategorySummaryExcel(counts, categoryOptions, documents)}
        >
          ⬇ Xuất Excel báo cáo tổng hợp danh mục
        </button>
      </div>
      ...
      <div className="view-head" style={{ marginTop: "2.5rem" }}>
        <h2 className="sub-title">Sơ đồ liên kết văn bản</h2>
        <p className="view-sub">Văn bản nào sẽ có liên kết thay thế / được thay thế bởi văn bản nào</p>
      </div>
      <LinkMindMap documents={documents} />
    </div>
  );
}
        ...
        <div className="history-timeline">
          {historyDoc ? (
            <>
              ...
              <p className="muted mono">{historyDoc.code}</p>
...
.nav-item:hover { background: var(--bg-sidebar-hover); color: var(--text-light); }
.nav-item.active { background: var(--bg-sidebar-hover); color: var(--text-light); border-left: 2px solid var(--accent-amber); }
...
.table-head, .table-row {
  display: grid; grid-template-columns: 1.1fr 2.4fr 1.4fr 1.2fr;
...
.modal-grid .v-link {
  display: block; width: 100%; text-align: left; border: none; background: none; padding: 0;
  font-size: 0.88rem; margin-top: 2px; color: var(--accent-blue); cursor: pointer; font-family: inherit;
}
.modal-grid .v-link:hover { text-decoration: underline; }
...
.related-picker {
  border: 1px solid var(--border); border-radius: 8px; max-height: 12rem; overflow-y: auto;
  padding: 0.5rem 0.75rem; background: #FAFBFC; margin-top: 0.35rem;
}
.related-picker-item { display: flex; align-items: flex-start; gap: 0.5rem; padding: 0.35rem 0; font-weight: 400; font-size: 0.85rem; cursor: pointer; }
.related-picker-item input[type="checkbox"] { margin-top: 0.2rem; flex-shrink: 0; }
...
.related-tags { margin-top: 1rem; }
.related-tags-title { display: block; font-size: 0.74rem; text-transform: uppercase; letter-spacing: 0.5px; color: var(--steel); margin-bottom: 0.5rem; }
.related-tags-list { display: flex; flex-wrap: wrap; gap: 0.5rem; }
.related-tag {
  border: 1.5px solid var(--node-color, var(--border)); color: var(--node-color, var(--text-dark));
  background: #fff; border-radius: 999px; padding: 0.3rem 0.75rem; font-size: 0.78rem; cursor: pointer; font-family: inherit;
}
.related-tag:hover { background: var(--node-color, var(--accent-blue)); color: #fff; }
...
.mindmap-wrap { margin-top: 1rem; }
.mindmap-picker { display: flex; flex-direction: column; gap: 0.4rem; font-size: 0.85rem; font-weight: 600; color: var(--text-dark); max-width: 26rem; margin-bottom: 1.5rem; }
.mindmap { display: flex; align-items: stretch; flex-wrap: wrap; gap: 0.75rem; }
.mindmap-node-row { display: flex; align-items: center; gap: 0.75rem; }
.mindmap-node {
...
.mindmap-node.active { background: var(--node-color, var(--accent-blue)); border-color: var(--node-color, var(--accent-blue)); }
.mindmap-node.active .mindmap-node-title,
.mindmap-node.active .mindmap-node-meta { color: #fff; }
.mindmap-node-title { font-weight: 600; font-size: 0.82rem; margin: 0.4rem 0 0.25rem; line-height: 1.35; }
.mindmap-node-meta { font-size: 0.72rem; color: var(--steel); }
.mindmap-arrow { color: var(--accent-blue); font-size: 1.2rem; flex-shrink: 0; }
.mindmap-related-list { display: flex; flex-wrap: wrap; gap: 0.5rem; }
.mindmap-related-chip {
  border: 1.5px dashed var(--node-color, var(--border)); color: var(--node-color, var(--text-dark));
  background: #fff; border-radius: 999px; padding: 0.35rem 0.8rem; font-size: 0.78rem; cursor: pointer; font-family: inherit;
}
.mindmap-related-chip:hover { background: var(--node-color, var(--accent-blue)); color: #fff; }
@media (max-width: 700px) {
  .mindmap { flex-direction: column; }
  .mindmap-node-row { flex-direction: row; }
}
